...
export class EditarProductComponent implements OnInit {
  ...
  updateProduct(): void {
    if (this.product.length > 0) {
      ...
      const selectedCategory = this.categories.find(category => category.categoryId === this.product[0].Category.categoryId);
      if (selectedCategory) {
        this.product[0].Category.categoryId = selectedCategory.categoryId;
      }
  
      // Buscar el proveedor seleccionado
      const selectedSupplier = this.suppliers.find(supplier => supplier.supplierId === this.product[0].Category.categoryId);
      if (selectedSupplier) {
        this.product[0].Supplier.supplierId = selectedSupplier.supplierId;
      }
